CREATE TABLE ohdsi_cumc_2021q1r2.results.diabetes_example (
...
( 
  select concept_id from ohdsi_cumc_2021q1r2.dbo.CONCEPT where concept_id in (1529331,1530014,44816332,43013884,19035533,43526465,19033498,1594973,44785829,45774435,45774751,1583722,19059796,1597756,1560171,19097821,1559684,19001441,40239216,40170911,44506754,1503297,1510202,1502826,19033909,1525215,1517998,1516766,1547504,40166035,793143,1580747,1502809,1502855,1515249,19122137)
...
( 
  select concept_id from ohdsi_cumc_2021q1r2.dbo.CONCEPT where concept_id in (201820,201826,443734,4226238,201531,443735,192279,443767,443730,321822)
UNION  select c.concept_id
  from ohdsi_cumc_2021q1r2.dbo.CONCEPT c
  join ohdsi_cumc_2021q1r2.dbo.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (201820,201826,443734,192279,443767,443730,321822)
  and c.invalid_reason is null

) I
LEFT JOIN
(
  select concept_id from ohdsi_cumc_2021q1r2.dbo.CONCEPT where concept_id in (193323,195771,194700,443012,4062685,4062686,40484648,201254,435216)
UNION  select c.concept_id
  from ohdsi_cumc_2021q1r2.dbo.CONCEPT c
  join ohdsi_cumc_2021q1r2.dbo.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (193323,195771,194700,443012,4062685,40484648,201254,435216)
  and c.invalid_reason is null
...
JOIN ohdsi_cumc_2021q1r2.dbo.OBSERVATION_PERIOD OP on Q.person_id = OP.person_id 
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,1,P.START_DATE) AND A.START_DATE <= DATEADD(day,730,P.START_DATE) ) cc 
...
DELETE FROM ohdsi_cumc_2021q1r2.results.diabetes_example where cohort_definition_id = 33333;
...
select 33333 as cohort_definition_id, person_id, start_date, end_date 
...
-- select * from ohdsi_cumc_2021q1r2.results.diabetes_example